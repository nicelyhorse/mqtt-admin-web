...
def Register(request):
    if request.method == 'POST':  #  请求方法为POST时，进行处理
        ...
        if len(data) == 0:
            ...
            result = 'This account has been registered, please replace it'
...
def Log_in(request):
    if request.method == 'POST':  #  请求方法为POST时，进行处理
        ...
        data = findUser(account)
        if len(data) == 0:
            result = 'Account input error, please check'
        else:
            if data[0][2] == pwd:
                result = 1
                ...
                result = 'Password error, please check'
        return_json = {
            'result': result,
            'url': 'Home'
        }
...
def Home(request):
    if 'name' in request.session:
        ...
        return render(request, 'You have not logged in yet, please log in')
        ...
        if order == "changeAccount":
            ...
            if account[-12:] == '@myemail.com' and len(account) > 12:
                ...
                if len(name) < 1 or len(pwd) < 1:
                    result = "The information format (all field lengths must be greater than 1) is incorrect. Please modify and try again"
                else:
                    # 查重
                    data = findUser(account)
                    if len(data) > 0:
                        result = 'The domain name has already been registered, please try again'
                    else:
                        sql = "UPDATE user_list set user_name='{}',account='{}',pwd='{}' where account='{}';". \
                            format(name, account, pwd, request.session['account'])
                        my_custom_sql(sql)
                        result = 'Successfully modified, click OK to log in again.'
            else:
                result = 'The domain name does not comply with the rules, and the format of the domain name is **** @ myemail.com'
            return_json = {
                'result': result,
                '': 1,
            }
...
def writeEmail(request):
    if request.method == 'POST':
        ...
        if order == "writeEmail":
            ...
            data = findUser(nto)
            if len(data) < 1:
                result = 'Sending failed, the email address has not been registered yet'
            else:
    ...
    if request.method == 'POST':
        ...
        if order == "connect":
            ...
            if len(nname) < 1 or len(naccount) < 1:
                result = 'Please complete the information'
            else:
